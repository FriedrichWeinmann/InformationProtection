...
$parentModule = 'InformationProtection'
if (-not $ModuleName) { $ModuleName = 'InformationProtection.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\InformationProtection'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\InformationProtection.psd1"
$prereqHash = @{
